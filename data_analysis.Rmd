...
<center>
<b>Toxicogenomic identification of repositioned therapy for a rare monogenic disease.</b>

Eric J. Kort MD MS, Nazish Sayed MD PhD, Chun Liu PhD, Sean Wu MD PhD, 
Joseph Wu MD PhD, Stefan Jovinge MD PhD.
...
rownames(data) <- ids
saveRDS(data, file="data/lamin.rds")
...
the LMNA set) in the LINCS data.  This calculation was performed exactl as 
above except that these random sample sets were used in place of the LMNA 
perturbed sample. This data then allowed us to empirically
...
```{r}
## Estimating significance of perturbed genes
...
fig2b <- function() {
  ...
  a <- ggplot(dat, aes(x = value, y = adj_p, color = Direction)) +
...
# the result of the above was saved in lincs_fda_zscores.rds
...
fig3ab <- function() {
  ...
  spec_p <- spec_p[-which(is.na(names(spec_p)))]
  ...
  cmap <- ggplot(df, aes(x, y)) +
    ...
    geom_point(aes(color = CMAP, fill = CMAP), shape = 21) +
    geom_text(
      data = labels_c,
      hjust = 0,
      aes(x, y, label = label),
  ...
  arb_sh <- ggplot(df, aes(x, y)) +
    ...
    geom_point(aes(color = ARB, fill = ARB), shape = 21) +
    geom_text(
      data = labels_h,
      hjust = 0,
      aes(x, y, label = label),
